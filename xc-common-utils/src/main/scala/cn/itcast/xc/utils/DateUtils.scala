...
object DateUtils {
  ...
  def getMonthInterval(num: Int): String = {
    ...
    println("月: " + (calendar.get(Calendar.MONTH) + 1) + " 开始日期: " + ym + startDay + " 结束日期: " + ym + endDay)
  ...
  def getWeekRelativelyInterval(date_info: Date): String = {
    ...
    // 先判断是不是周一
    if (1 == week) {
      return getWeekInterval(1, date_info) + "," + getWeekInterval(2, date_info)
    }
  ...
  def getLastWeekIntervalWithMillis(num: Int): String = {
    ...
    println("周: " + calendar.get(Calendar.WEEK_OF_YEAR) + " 开始日期: " + weekStartDay + " 结束日期: " + weekEndDay)
    // 返回毫秒值
   ...
   * @return
   */
  def getWeekInterval(num: Int, date_info: Date): String = {
    val sdf: SimpleDateFormat = new SimpleDateFormat("yyyy-MM-dd")
    val calendar = Calendar.getInstance()
    calendar.setTime(date_info)
    calendar.setFirstDayOfWeek(Calendar.MONDAY)
